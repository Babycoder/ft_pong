<template>
  ...
  <div class="flex flex-row h-full">
    ...
    <div class="px-16 py-4 text-gray-700 bg-gray-200 h-screen w-screen">
        <!-- Content -->
        <router-view v-show="validUrl"/>
        <ErrorPageBlock v-show="!validUrl" />
    </div>
    <!-- <testApp  /> -->
  </div>
</template>
...
export default defineComponent({
  ...
  computed :{
      validUrl() : boolean
      {
        ...
        return (current === '/' || current === 'profile' || current === 'users' || current === "leaderboard" || current === 'matchhistory' || current === 'game'
         || current === 'chat' || current === 'Add' || current === 'Ignore' || current === 'Requests' || current === 'FriendProfile')
      }
  },
  methods: {
    // test()
